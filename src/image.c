...
/*   Updated: 2023/08/28 20:57:48 by fras          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
mlx_image_t	*draw_fractal(mlx_image_t *fractal, t_canvas canvas, uint32_t color_scheme[])
{
	...
	while (pixel_pos[Y] < HEIGHT)
	{
		...
		{
			in_set_check = calculate_fractal(canvas_pos[X], canvas_pos[Y], canvas);
...
void	put_pixels(mlx_image_t *fractal, int pixel_pos[], int in_set_check, uint32_t color_scheme[])
{
	...
	if (in_set_check == psychedelic_depth)
		...
		mlx_put_pixel(fractal, pixel_pos[X], pixel_pos[Y], color_scheme[in_set_check]);
